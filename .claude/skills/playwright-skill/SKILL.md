...
1. **Antes de escrever script solto, considere as alternativas** — esta skill é
   para automação exploratória e pontual. Se o objetivo é regressão, o lugar é a
   suíte do projeto (ver "Relação com o resto do harness" no fim deste arquivo).
...
6. **Promover para regressão** — Se o cenário deve ser verificado sempre, consolide
   em `apps/web/e2e/*.spec.ts` (runner `@playwright/test`, fixtures em
   `apps/web/e2e/fixtures.ts`) e rode `npm run e2e -w orbien-web` a partir da raiz.
...
Antes de acionar esta skill, verifique se uma das duas opções mais baratas resolve:

| Objetivo | Use |
|---|---|
| Inspecionar página, clicar, ler DOM, ver console e rede, tirar screenshot | As **ferramentas de navegador embutidas** do Claude Code (`navigate`, `computer`, `read_page`, `read_console_messages`, `read_network_requests`). Não exigem instalação nem escrever script |
| Verificar fluxo autenticado do app, de forma repetível | A **suíte do projeto** em `apps/web/e2e/` — já tem sessão semeada por HTTP, fixtures que se limpam, e trace na falha |
| Automação exploratória fora do app, ou cenário que a suíte não cobre | Esta skill |

Esta skill exige `npm install` e download de browser **dentro da própria pasta**
(`npm run setup`), separados dos do projeto. Se você só precisa olhar uma página,
não vale o custo.
