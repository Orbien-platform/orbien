...
- Instruções de agente: `CLAUDE.md` da raiz, `apps/*/AGENTS.md`
- Contributor guides: `CONTRIBUTING.md`, `docs/` (testing, quality, or standards subdocs), README testing section
- Configuração de teste: `apps/api/jest.config.js`, `apps/web/playwright.config.ts`
...
2. **Descubra os comandos no repo, não invente.** Este é um monorepo npm workspaces + Turborepo: os comandos estão no `package.json` da raiz (`npm run build:api`, `build:web`, `build:site`, `test`, `lint`) e nos `apps/*/package.json`. Rode sempre a partir da raiz.
...
| Code Layer | Required Test Type | Coverage Expectation | Location Pattern | Run Command |
| ---------- | ------------------ | -------------------- | ---------------- | ----------- |
| Serviço/regra da API | unit | 1:1 com os ACs; casos de borda listados | `apps/api/test/**/*.spec.ts` | `npm run test -w orbien-backend` |
| Isolamento multi-tenant | RLS | Toda tabela nova: leitura e escrita cross-tenant | `apps/api/test/rls/isolation.spec.ts` | `npm run test:rls -w orbien-backend` |
| Fluxo de tela | e2e | Caminho feliz + um de erro | `apps/web/e2e/*.spec.ts` | `npm run e2e -w orbien-web` |

> O jest da API usa `testMatch: ['<rootDir>/test/**/*.spec.ts']`. Teste colocado
> em `src/**/__test__/` **não é executado** — não co-loque aqui.
