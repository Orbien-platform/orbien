...
- As HARD CONSTRAINTS de integridade de teste, acima, valem aqui igualmente
...
Um commit por task, imediatamente após a verificação. Nunca agrupe tasks num commit.

As convenções de mensagem deste repositório — tipos em uso, idioma, escopo — ficam no
`CLAUDE.md` da raiz e no próprio histórico (`git log`). Siga o que está lá; não há
convenção própria desta skill.

Só commite depois do gate verde. Não inclua arquivo fora do escopo da task.
...
2. **Discrimination sensor** — injects a small behavior-level fault (flip a condition, change a return value, off-by-one) in a scratch/throwaway state (git worktree descartável ou cópia temporária — nunca `git stash`, que é global), runs the relevant tests, confirms they kill the mutant, then discards the mutation. Reports killed/survived; surviving mutants become fix tasks.
...
### 11. Handoff to PR (Fill — after Verifier PASS)

When the Verifier returns **PASS** (or standalone validation passes) and the user wants a pull request, hand off to the `pull-request` skill — do not invent an alternate PR template. Ela lê o diff real e o `.specs/` para redigir título e corpo.

Surface once, conversationally: "Validação passou — quer que eu redija o PR com a skill `pull-request`?" Skip if the user already declined or is continuing with more features.
