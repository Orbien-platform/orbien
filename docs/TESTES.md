...
138 testes e deve falhar alto se eles sumirem. Os outros precisam da flag
...
`DATABASE_URL`/`DIRECT_URL`; no CI ele continua verde com os 138 testes. Para
...
> 2026-09-15 (o `PROD-20` trouxe `test/rls/networks.spec.ts`) e para **138** em
> 2026-09-16 (as ações A e B do `PEND-04` trouxeram
> `test/rls/auth-tables.spec.ts` (8) e `test/rls/audit-writes.spec.ts` (5)).
>
...
**Não está mais**: `npm run test:rls -w orbien-backend` fecha em 138 testes
verdes (o plano falava em 39, depois 54, 118, 125, 133 — a suíte cresceu desde
