...
### PEND-09 · Env vars de produção do provedor bíblico não configuradas · dívida
...
(`BIBLE_API_BASE_URL`, `BIBLE_API_VERSION_ID`), com os valores de exemplo já
em `apps/api/.env.example`. O que falta é humano: colar essas duas no Render
— não há como este código fazer isso sozinho.

Nasceu como pendência de token (o provedor era abibliadigital.com.br, com
conta e chave por usuário). Em 2026-09-21 o time do provedor migrou para
abibliadigital.api.br: API pública sem autenticação, WAF e rate limit via
Cloudflare no lugar do token. `ApiBibleTextProvider` e `.env.example` já
foram atualizados para o domínio novo e pararam de usar `BIBLE_API_KEY` —
o que falta encolheu de "criar conta e gerar token" para "copiar duas
variáveis de config pública para o Render". Checklist em `DEPLOY.md`, seção
1.4. Fechar antes de abrir a feature para usuário de verdade — hoje ela
